...
def train_model(model_type: str):
    ...
    with mlflow.start_run():
        if model_type == 'random_forest':
            ...
            mlflow.log_artifact("models/random_forest.metadata.json")
        elif model_type == 'random_forest_v2':
            model, metadata = train_random_forest_classifier_v2(DATA_FILE)
            mlflow.log_artifact("models/random_forest_v2.metadata.json")
        elif model_type == 'logistic_regression':
            model, metadata = train_logistic_regression_classifier(DATA_FILE)
            mlflow.log_artifact("models/logistic_regression.metadata.json")
